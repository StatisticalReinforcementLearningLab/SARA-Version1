...
app.directive("w3TestDirective", function($rootScope,saraDatafactory) {
    return {
        ...
        link : function(scope, element, attrs, injector,location,http,rootScope){
            ...
            saraDatafactory.pullGameScoreData(function(returnValue) {
            ...
            function updateTheScore(scoreValue){
                  ...
                  console.log("Updating score " + scoreValue);
...
app.controller("MainCtrl", function($scope,$state,$rootScope,$ionicPlatform,$ionicPopup,$timeout,$location,$cordovaStatusbar,$cordovaInAppBrowser,$interval,$rootScope,saraDatafactory) {
    ...
    *********************************************
    ...
    AWS.config.credentials.get(function(){
       ...
       syncClient.openOrCreateDataset('datasetname2', function(err, dataset2) {

          //
          var cache = [];
          var x = JSON.stringify(dataset2, function(key, value) {
          ...
          //Synchronize 
          dataset2.synchronize({

            onSuccess: function(dataset, newRecords) {
               //...
               dataset2.get('myKey',function(err, value) {
    ...
    */
    ...
    getActiveTaskData();
    var promise = $interval(isLoggedIn, 2000);
    ...
    function isLoggedIn() {
      ...
      if($rootScope.isConnectedWithApp == false){
          $scope.$broadcast('game:updateIsConnected',{state: true});
      }else{
          $scope.$broadcast('game:updateIsConnected',{state: false});
          //$interval.cancel();
      }
      ...
      if(($rootScope.activeTaskAdded != undefined) && $rootScope.activeTaskAdded ){
          $scope.$broadcast('game:addscore', {state: 15*$rootScope.activeTaskAddedHowMany});
    ...
    $scope.changeLevel = function(level) {
      $scope.$broadcast('game:level', {state: level});//SUCCESS  
    };
    ...
    $scope.startActiveTask = function() {
        ...
        if(device.platform === 'Android') {
            var sApp = startApp.set({ /* params */
              ...
              "component": ["edu.stat.srl.passivedatakit","edu.stat.srl.passivedatakit.MainActivity"]
            }, { /* extras */
    ...
    function showPopup() {
        ...
        var myPopup = $ionicPopup.show({
          ...
          template: '<button class="button button-full button-positive" ng-click="startDailySurvey()">Daily Survey</button><button class="button button-full button-positive" ng-click="startActiveTask()">Active Task</button><button class="button button-full button-positive" ng-click="startWeeklySurvey()">Weekly Survey</button>',
