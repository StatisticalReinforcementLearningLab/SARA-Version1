    ...
    for j in range(len(all_datasets)):
        ...
        if all_datasets[j][u'DatasetName'] == 'rl_data':
            ...
            if identityId != 'us-east-1:213a76cd-de58-4067-bd3b-6f9ac0547cd7':
                ...
                if not os.path.exists(COG_BACKUP_DIRECTORY + '/' + directory_name + '/complete_data'):
                    os.makedirs(COG_BACKUP_DIRECTORY + '/' + directory_name + '/complete_data')
                #
                d = datetime.today()
                ...
                for ii in range(len(response[u'Records'])):
                    #
                    if u'Value' not in response[u'Records'][ii]:
                        continue

                    t_response = json.loads(response[u'Records'][ii][u'Value'])
                    #pdb.set_trace()
                    t_date = str(response[u'Records'][ii][u'Key'])
                    json_file = open(COG_BACKUP_DIRECTORY + '/' + directory_name + '/complete_data/' + t_date + ".txt","w")
                    json_file.write(json.dumps(t_response, indent=4, sort_keys=False))
                    json_file.close()
