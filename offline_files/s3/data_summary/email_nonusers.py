...
body3 = '<h2 style="padding:0; margin: 0;">SARA adherence report: </h2>'
...
for i in range(len(all_identities)):
            ...
            if identityId != 'us-east-1:213a76cd-de58-4067-bd3b-6f9ac0547cd7':
                ...
                for k in range(30):
                    ...
                    if logged_count > 0:        
                        ...
                        logging_array.append(1)      
                    else:
                        logging_string = logging_string + '-'
                        logging_array.append(0)    
    ...
    # we don't care for less than one day    
    if len(logging_array) < 2:
        continue

    total_log_count_2_days = logging_array[0] + logging_array[1]
    if total_log_count_2_days > 0:
        continue # we will only who ever logged

    #last logged
    how_many_days_ago = 0
    for i in range(len(logging_array)):
        if logging_array[i] == 0:
            how_many_days_ago = how_many_days_ago + 1
        else:
            break
    how_many_days_ago = str(how_many_days_ago)

    logging_string = logging_string[::-1]    
    #if round(100*(total_ds_7+total_at_7)/14.0,2) < 50:
    if "-study-" in username:
        first_table_body = first_table_body + '<tr><td style="padding:8px; text-align: left;">' + username  + '</td><td style="padding:8px; text-align: left;">' + str(total_days) + '</td><td style="padding:8px; text-align: left;">' + data_today + '</td><td style="padding:8px; text-align: left;">' + data_7 + '</td><td style="padding:8px; text-align: left;">' + last_logged_date + '</td><td style="padding:8px; text-align: left;">' + how_many_days_ago + ' days ago</td><td style="padding:8px; text-align: left;">' + logging_string + '</td></tr>' 
    #else:
    #    first_table_body = first_table_body + '<tr style="color: #82B1FF;"><td style="padding:8px; text-align: left;"><b>' + username + '</b></td><td style="padding:8px; text-align: left;"><b>' + str(total_days) + '</b></td><td style="padding:8px; text-align: left;"><b>' + data_today + '</b></td><td style="padding:8px; text-align: left;"><b>' + data_7 + '</b></td><td style="padding:8px; text-align: left;"><b>' + last_logged_date + '</b></td><td style="padding:8px; text-align: left;"><b>' + how_many_days_ago + ' days ago</b></td><td style="padding:8px; text-align: left;"><b>' + logging_string + '</b></td></tr>'
    i = i + 1



body2 = '<br><br><h3>Summary</h3><b>Stored as: </b> (no_of_survey/no_of_active_tasks)'
body2 = body2 + '<table style="width:1000px; border-collapse: collapse;">'
...
body2 = body2 + '<th style="padding:8px; text-align: left;background-color: #D32F2F;color: white;">Last logged<br>date</th>'
body2 = body2 + '<th style="padding:8px; text-align: left;background-color: #D32F2F;color: white;">Last<br>Logged</th>'
body2 = body2 + '<th style="padding:8px; text-align: left;background-color: #D32F2F;color: white;">Logging over days<br> "|" is yes, "-" is no, (first day to today)</th></tr>'
body2 = body2 + first_table_body
body2 = body2 + '</table><br><br>'

body = header + "<body>" + body3 + body2 + "</body></html>"

d = datetime.today()
date_str = d.strftime ("%Y%m%d")

file = open(date_str + '_adherence.html', 'w')
file.write(body)
file.close()
...
recipient = 'mrabbi@umich.edu'
subject = 'SARA adherence update (' + time.strftime("%b %d, %Y") + ")"
subject_base = 'SARA adherence update (' + time.strftime("%b %d, %Y") + ")"
cc = ['mrabbi@umich.edu','mphilyaw@med.umich.edu','sarasquad@med.umich.edu']
headers = ["From: " + sender,
           "Subject: " + subject,
           "To: " + recipient,
           "CC: " + ",".join(cc),
...
session.starttls()
session.ehlo 
session.login('mashfiqui.r.s', 's7h1u2v5a029')
session.sendmail(sender, [recipient] + cc, headers + "\r\n\r\n" + body)
session.quit()



file = open("/var/www/html/dailyupdates/" + date_str + '_adherence.html', 'w')
file.write(body)
file.close()
